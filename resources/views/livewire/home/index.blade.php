<div class="text-zinc-100">
    @section('breadcrumb')
        <span class="text-zinc-400">Dashboard</span>
    @endsection

    {{-- Conteúdo (já está dentro do <main class="app-container app-main"> do layout) --}}
    <section class="surface p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold">
                    Bem-vindo ao <span class="text-edp-iceblue-100">SicodeSK</span>
                </h1>
                <p class="mt-1 text-zinc-400">
                    Abra, acompanhe e conclua tickets.
                </p>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('tickets.create') }}"
                    class="rounded-lg px-4 py-2.5 text-sm font-medium border border-blue-600 bg-blue-600 hover:bg-blue-700 hover:border-blue-700 transition-colors w-32 text-center"
                    wire:navigate>
                    + Novo ticket
                </a>
                <a href="{{ route('tickets.index') }}"
                    class="rounded-lg px-4 py-2.5 text-sm font-medium border border-[#2b3649] bg-[#0f172a] hover:bg-[#1a2436] hover:border-[#374151] transition-colors w-32 text-center"
                    wire:navigate>
                    Meus tickets
                </a>
            </div>
        </div>
    </section>

    {{-- KPIs --}}
    <section class="mb-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            @foreach ($kpis as $kpi)
                <div class="rounded-xl border border-[#2b3649] bg-[#1b2535] p-4">
                    <div class="text-sm text-zinc-400">{{ $kpi['label'] }}</div>
                    <div class="mt-2 text-3xl font-semibold {{ $kpi['accent'] ?? '' }}">{{ $kpi['value'] }}</div>
                    <div class="mt-1 text-xs text-zinc-500">{{ $kpi['muted'] }}</div>
                </div>
            @endforeach
        </div>
    </section>

    {{-- Tickets Recentes (Livewire) --}}
    <section class="mt-8">
        <livewire:tickets.recent />
    </section>
</div>
